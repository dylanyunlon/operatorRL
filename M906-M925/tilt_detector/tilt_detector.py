...
Architecture Pattern:
  ...
  遵循该模式实现 TiltDetector,
  让 operatorRL 可以Detect opponent tilt state from recent game outcomes,
...
class TiltDetector:
    ...
    def __init__(self, connector=None, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info("TiltDetector initialized (deps=['M906', 'M907', 'M908'])")
    ...
    async def analyze(self, input_data: Dict[str, Any]) -> AnalysisResult:
        ...
        try:
            ...
            logger.error("TiltDetector analysis error: %s", exc)
    ...
    async def reset(self) -> None:
        ...
        self._process_count = 0
        self._error_count = 0
        logger.info("TiltDetector reset")

    def __repr__(self) -> str:
        return f"TiltDetector(state={self._state.value}, processed={self._process_count})"
